...
class Game
  ...
  def valid_comp_coordinates(ship)
    coordinates = []
    coordinates = @comp_board.cells.keys.sample(ship.length) until @comp_board.valid_placement?(ship, coordinates)
    coordinates
  end
  ...
  def player_fire
    ...
    if @comp_board.valid_coordinate?(player_shot) == false
      ...
      case @comp_board.cells[player_shot].render
      when 'M'
      ...
      when 'H'
        ...
        @comp_board.cells[player_shot].render == 'X'
  ...
  def player_win?
    return unless (@comp_cruiser.sunk? == true) && (@comp_sub.sunk? == true)
  ...
  def comp_win?
    return unless (@player_cruiser.sunk? == true) && (@player_sub.sunk? == true)
